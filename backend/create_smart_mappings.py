...
def load_all_statements():
    ...
    for file_path in sorted(excel_files):
        try:
            ...
            if 'Acct_Statement' in file_name:  # HDFC Account Statements
                ...
                for start_idx in range(len(df)):
                    ...
                    if any(x in row_str for x in ['date', 'description', 'withdrawal', 'deposit']):
                        df = pd.read_excel(file_path, sheet_name=0, header=start_idx)
                        break
            ...
            elif 'CC_Statement_2026' in file_name:  # Recent CC Statement (Well-formatted)
                # This one has proper headers
                for _, row in df.iterrows():
                    title = str(row['Transaction Details']).strip() if pd.notna(row['Transaction Details']) else ''
                    try:
                        amount_val = str(row['Amount (INR)']).replace(',', '').replace('₹', '').strip()
                ...
                for _, row in df.iterrows():
                    ...
                    for col in df.columns:
                        if 'transaction' in str(col).lower() or 'narration' in str(col).lower():
                            title = str(row[col]).strip() if pd.notna(row[col]) else None
                        elif 'amount' in str(col).lower():
                            try:
                                amount = float(str(row[col]).replace(',', '').replace('₹', '').strip())
                            except:
                                pass
...
def find_best_match(statement_title, clean_title, amount, reference_df):
    ...
    if len(exact_matches) > 0:
        ...
        for _, ref_row in exact_matches.iterrows():
            ref_title = str(ref_row['title']).strip().lower() if pd.notna(ref_row['title']) else ''
            if ref_title == clean_title.lower():
                return ref_row['title']
        ...
        for _, ref_row in exact_matches.iterrows():
            ...
            # Remove common words
            common_words = {'the', 'and', 'or', 'a', 'an', 'in', 'on', 'at', 'to', 'for', 'of', 'by', 'transfer', 'dr', 'cr', 'payment', 'funds', 'account'}
    ...
    if clean_title and clean_title != statement_title:
        ...
        for _, ref_row in valid_ref_df.iterrows():
            ...
            similarity = SequenceMatcher(None, clean_title.lower(), ref_title).ratio()
...
def create_merchant_mappings():
    ...
    try:
        ...
        for stmt in statements:
            ...
            mapped_title = find_best_match(statement_title, clean_title, amount, reference_df)
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (1, amount, statement_title, clean_title, mapped_title_display, datetime.utcnow().isoformat()))
